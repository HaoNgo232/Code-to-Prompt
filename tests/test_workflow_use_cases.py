...
def test_workflow_engine_basic_success(tmp_path: Path):
    ...
    assert (
        len(observer.events) == 6
    )  # workflow_start, step1_start, step1_complete, step2_start, step2_complete, workflow_complete
...
def test_workflow_engine_step_failure(tmp_path: Path):
    ...
    steps = [CallableWorkflowStep(name="bad_step", handler=bad_step)]
...
def test_build_context_use_case(fake_ws: Path):
    ...
    # Test map exception
    with patch(
        "domain.workflow.context_builder.run_context_builder",
...
def test_code_review_use_case(fake_ws: Path):
    ...
    cmd = CodeReviewCommand(workspace_path=fake_ws, review_focus="security")
    ...
    with patch(
        "domain.workflow.code_reviewer.run_code_review",
...
def test_refactor_use_case(fake_ws: Path):
    ...
    cmd_invalid_phase = RefactorCommand(
        workspace_path=fake_ws, refactor_scope="Refactor Model", phase="invalid"
    )
    with pytest.raises(
        UseCaseValidationError, match="phase must be 'discover' or 'plan'"
    ):
        use_case.execute(cmd_invalid_phase)

    cmd_missing_report = RefactorCommand(
        workspace_path=fake_ws,
    ...
    cmd_discover = RefactorCommand(
        workspace_path=fake_ws, refactor_scope="Refactor Model", phase="discover"
    )
    ...
    # Test plan execution
    cmd_plan = RefactorCommand(
        workspace_path=fake_ws,
        refactor_scope="Refactor Model",
        phase="plan",
        discovery_report="Done discovery",
    )
...
def test_investigate_use_case(fake_ws: Path):
    ...
    cmd = InvestigateCommand(
        workspace_path=fake_ws, bug_description="IndexError in loop"
    )
...
def test_test_build_use_case(fake_ws: Path):
    ...
    cmd = TestBuildCommand(workspace_path=fake_ws, task_description="Build tests")
...
def test_design_planner_use_case(fake_ws: Path):
    ...
    cmd = DesignPlannerCommand(
        workspace_path=fake_ws, task_description="Design database model"
    )
