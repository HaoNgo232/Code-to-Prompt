...
from presentation.components.dialogs.exclusions_dialog import (
    ExclusionsDialog,
    PatternRow,
)
...
def test_custom_template_dialog_create(qtbot, tmp_path):
    ...
    with patch(
        "presentation.components.dialogs.custom_template_dialog.CUSTOM_TEMPLATES_DIR",
        mock_dir,
    ):
        ...
        assert (
            "<!-- name: Reviewer Custom, desc: A custom code reviewer template -->"
            in content
        )
