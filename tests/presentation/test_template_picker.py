...
def test_picker_shows_exactly_7_builtin(context_view):
    ...
    with patch(
        "domain.prompt.template_manager.list_templates", return_value=builtin_templates
    ):
...
def test_no_tier_ui_elements_present(context_view):
    ...
    for widget in view.findChildren(QWidget):
        ...
        assert not re.search(r"\b(lite|pro|tier)\b", text), (
            f"Found invalid tier word in widget text: {text}"
        )
        assert not re.search(r"\b(lite|pro|tier)\b", tooltip), (
            f"Found invalid tier word in widget tooltip: {tooltip}"
        )
    ...
    for action in view._template_menu.actions():
        ...
        assert not re.search(r"\b(lite|pro|tier)\b", text), (
            f"Found invalid tier word in action text: {text}"
        )
        assert not re.search(r"\b(lite|pro|tier)\b", tooltip), (
            f"Found invalid tier word in action tooltip: {tooltip}"
        )
...
def test_custom_templates_shown_at_bottom(context_view):
    ...
    templates = [
        TemplateInfo(
            template_id="b_1",
            display_name="Builtin 1",
            description="D1",
            is_custom=False,
        ),
        TemplateInfo(
            template_id="b_2",
            display_name="Builtin 2",
            description="D2",
            is_custom=False,
        ),
        TemplateInfo(
            template_id="c_1", display_name="Custom 1", description="D3", is_custom=True
        ),
        TemplateInfo(
            template_id="c_2", display_name="Custom 2", description="D4", is_custom=True
        ),
    ]
...
def test_template_selection_injects_to_instructions(context_view):
    ...
    with patch(
        "domain.prompt.template_manager.load_template",
        return_value="Refactor this code to follow SOLID.",
    ) as mock_load:
        view._on_template_selected(mock_action)
        mock_load.assert_called_once_with("my_template_id")

    assert (
        view._instructions_field.toPlainText() == "Refactor this code to follow SOLID."
    )
...
def test_no_hardcoded_template_count(context_view):
    ...
    with patch(
        "domain.prompt.template_manager.list_templates", return_value=templates_3
    ):
        view._populate_template_menu()

    actions = [
        a
        for a in view._template_menu.actions()
        if not a.isSeparator() and a.text() != "Manage/Add Custom Template..."
    ]
    ...
    with patch(
        "domain.prompt.template_manager.list_templates", return_value=templates_10
    ):
        view._populate_template_menu()
