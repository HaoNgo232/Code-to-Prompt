...
from PySide6.QtWidgets import QMessageBox, QFileDialog, QDialog, QLineEdit, QPushButton, QComboBox
...
class DummyMCPInstaller:
    ...
    def install_config(self, target_name: str, workspace_path: str = None) -> tuple[bool, str]:
        return True, "Success"
...
def setup_registry_dependencies():
    ...
    try: orig_settings_service = DomainRegistry.settings_service()
    except RuntimeError: pass
    try: orig_settings_provider = DomainRegistry._settings_provider
    except AttributeError: pass
    try: orig_session_state = DomainRegistry.session_state()
    except RuntimeError: pass
    try: orig_mcp_installer = DomainRegistry.mcp_installer()
    except RuntimeError: pass
    try: orig_ai_factory = DomainRegistry.ai_provider_factory()
    except RuntimeError: pass
...
def test_settings_view_reset_settings(qtbot, setup_registry_dependencies):
    ...
    with patch.object(QMessageBox, "warning", return_value=QMessageBox.StandardButton.Cancel) as mock_warn:
...
def test_settings_view_clear_session(qtbot):
    ...
    # 1. User Cancels
    with patch.object(QMessageBox, "question", return_value=QMessageBox.StandardButton.Cancel) as mock_quest, \
         patch("presentation.views.settings.settings_view_qt.clear_session_state") as mock_clear:
        view._clear_session()
    ...
    with patch.object(QMessageBox, "question", return_value=QMessageBox.StandardButton.Yes), \
         patch("presentation.views.settings.settings_view_qt.clear_session_state", return_value=True) as mock_clear:
        view._clear_session()
...
def test_settings_view_export_import_settings(qtbot):
    ...
    # Export settings
    with patch("presentation.views.settings.settings_view_qt.copy_to_clipboard", return_value=(True, "")) as mock_copy:
    ...
    with patch("presentation.views.settings.settings_view_qt.get_clipboard_text", return_value=(True, "")) as mock_get_empty:
        view._import_settings()
        mock_get_empty.assert_called_once()

    with patch("presentation.views.settings.settings_view_qt.get_clipboard_text", return_value=(True, "{invalid-json}")):
        view._import_settings()

    with patch("presentation.views.settings.settings_view_qt.get_clipboard_text", return_value=(True, '{"some_other_key": 1}')):
        view._import_settings()
    ...
    with patch("presentation.views.settings.settings_view_qt.get_clipboard_text", return_value=(True, json.dumps(import_data))), \
...
def test_settings_view_install_mcp(qtbot, setup_registry_dependencies):
    ...
    with patch.object(QDialog, "exec", return_value=QDialog.DialogCode.Accepted) as mock_exec:
        view._install_mcp_for("Cursor")
        mock_exec.assert_called_once()
        assert view._mcp_status_labels["Cursor"].text() == "✓ Installed"

    # 2. Workspace install flow for Copilot (workspace_only)
    with patch.object(QFileDialog, "getExistingDirectory", return_value="/mock/workspace") as mock_fd, \
         patch.object(QDialog, "exec", return_value=QDialog.DialogCode.Accepted) as mock_exec:
...
def test_settings_view_fetch_ai_models(qtbot, setup_registry_dependencies):
    ...
    with patch.object(QThreadPool.globalInstance(), "start", side_effect=lambda w: w.run()):
        view._fetch_ai_models()
