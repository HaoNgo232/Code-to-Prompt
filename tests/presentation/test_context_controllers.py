...
def mock_qt_threading():
    ...
    with patch("presentation.views.context.related_files_controller.schedule_background", lambda f: f()), \
         patch("presentation.views.context.related_files_controller.run_on_main_thread", lambda f: f()), \
         patch("presentation.views.context.tree_management_controller.run_on_main_thread", lambda f: f()), \
...
def test_tree_management_add_and_undo_ignore(qtbot):
    ...
    with patch("application.services.workspace_config.add_excluded_patterns", return_value=True) as mock_add:
        controller.add_to_ignore()
    ...
    # Undo ignore
    with patch("application.services.workspace_config.remove_excluded_patterns", return_value=True) as mock_remove:
        controller.undo_ignore()
        mock_remove.assert_called_once()
        assert set(mock_remove.call_args[0][0]) == {"file1.py", "sub/file2.py"}
        assert controller._last_ignored_patterns == []
...
def test_tree_management_dialogs(qtbot):
    ...
    with patch("presentation.components.dialogs.dialogs_qt.RemoteRepoDialogQt") as mock_dialog_cls:
    ...
    with patch("presentation.components.dialogs.dialogs_qt.CacheManagementDialogQt") as mock_dialog_cls:
        mock_instance = MagicMock()
        mock_dialog_cls.return_value = mock_instance
...
def test_tree_management_file_watcher_callbacks(qtbot):
    ...
    view._graph_provider.on_files_changed.assert_called_once_with(["/mock/workspace/file1.py"])
    
    # 2. File created
    view._graph_provider.on_files_changed.reset_mock()
    controller.on_file_created("/mock/workspace/new.py")
    view._graph_provider.on_files_changed.assert_called_once_with(["/mock/workspace/new.py"])
    
    # 3. File deleted
    controller.on_file_deleted("/mock/workspace/deleted.py")
    view._graph_provider.on_files_deleted.assert_called_once_with(["/mock/workspace/deleted.py"])
...
def test_related_files_resolving(qtbot):
    ...
    with patch("pathlib.Path.is_file", return_value=True), \
        ...
        with patch("presentation.views.context.related_files_controller.get_related_files_for_paths", return_value={"/mock/workspace/helper.py", "/mock/workspace/main.py"}) as mock_get:
...
def test_copy_action_cache_hit(qtbot):
    ...
    with patch("pathlib.Path.is_file", return_value=True), \
        ...
        with patch.object(QThreadPool, "start") as mock_start:
            ...
            view.clipboard_service.copy_to_clipboard.assert_called_once_with("Cached Prompt Output")
...
def test_copy_action_generation_guard(qtbot):
    ...
    with patch("pathlib.Path.is_file", return_value=True), \
        ...
        with patch("presentation.views.context.copy_action_controller.CopyTaskWorker") as mock_worker_cls:
