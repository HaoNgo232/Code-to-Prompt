...
def test_count_tokens_for_file_no_cache_edge_cases(tmp_path):
    ...
    with patch.object(Path, "stat") as mock_stat:
        mock_stat.return_value = os.stat_result((stat.S_IFREG, 0, 0, 0, 0, 0, MAX_BYTES + 1, 0, 0, 0))
...
def test_count_tokens_for_file_no_cache_exception_handling(tmp_path):
    ...
    with patch("domain.tokenization.counter.count_tokens", side_effect=Exception("error")):
