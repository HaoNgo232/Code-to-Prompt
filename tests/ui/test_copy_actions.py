...
def test_begin_copy_operation(context_view):
    ...
    view = context_view
    old_gen = view._copy_controller._copy_generation
...
def test_begin_copy_operation_moves_refs_to_stale(context_view):
    ...
    view._copy_controller._current_copy_worker = fake_worker
    view._copy_controller._current_copy_signals = fake_signals

    view._copy_controller._begin_copy_operation()

    assert view._copy_controller._current_copy_worker is None
    assert view._copy_controller._current_copy_signals is None
    assert fake_worker in view._copy_controller._stale_workers
    assert fake_signals in view._copy_controller._stale_workers
...
def test_cleanup_stale_refs(context_view):
    ...
    view._copy_controller._stale_workers = [mock_obj]
    view._copy_controller._cleanup_stale_refs(view._copy_controller._copy_generation)
...
def test_cleanup_stale_refs_empty(context_view):
    ...
    view = context_view
    view._copy_controller._stale_workers = []
    view._copy_controller._cleanup_stale_refs(
        view._copy_controller._copy_generation
    )  # Should not raise
...
def test_is_current_generation(context_view):
    ...
    view = context_view
    assert (
        view._copy_controller._is_current_generation(
            view._copy_controller._copy_generation
        )
        is True
    )
    assert (
        view._copy_controller._is_current_generation(
            view._copy_controller._copy_generation - 1
        )
        is False
    )
...
def test_set_copy_buttons_enabled(context_view):
    ...
    view = context_view
    view.set_copy_buttons_enabled(False)
    assert not view._copy_btn.isEnabled()

    view.set_copy_buttons_enabled(True)
...
def test_collect_all_tree_paths(context_view):
    ...
    paths = view._copy_controller._collect_all_tree_paths(root)
...
def test_show_copy_breakdown_basic(context_view):
    ...
    with patch("presentation.components.toast.toast_qt.toast_success") as mock_toast:
        ...
        assert "successful" in str(call_kwargs)
