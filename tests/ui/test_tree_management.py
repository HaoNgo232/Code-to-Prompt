...
def test_add_to_ignore(mock_add, context_view):
    ...
    assert len(view._tree_controller._last_ignored_patterns) > 0
...
def test_open_remote_repo_dialog(mock_repo_mgr, mock_dialog, context_view):
    ...
    view = context_view
    view._tree_controller._repo_manager = None
    mock_dialog_instance = MagicMock()
    mock_dialog.return_value = mock_dialog_instance

    view._tree_controller.open_remote_repo_dialog(view)

    assert view._tree_controller._repo_manager is not None
...
def test_open_remote_repo_dialog_existing_manager(mock_dialog, context_view):
    ...
    view._tree_controller._repo_manager = existing_manager
    mock_dialog_instance = MagicMock()
    mock_dialog.return_value = mock_dialog_instance
...
def test_open_cache_management_dialog(mock_repo_mgr, mock_dialog, context_view):
    ...
    view._tree_controller.open_cache_management_dialog(view)
...
def test_on_file_deleted_delegates(context_view):
    ...
    view = context_view
    view._tree_controller.on_file_modified = MagicMock()
    view._tree_controller.on_file_deleted("/fake/workspace/old.py")
    view._tree_controller.on_file_modified.assert_called_once_with(
        "/fake/workspace/old.py"
    )
...
def test_on_file_system_changed(mock_run, context_view):
    ...
    view = context_view
    view._tree_controller.on_file_system_changed()
