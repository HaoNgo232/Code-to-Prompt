...
class TestPreviewAnalyzerExtra:
    def test_analyze_file_actions_success_and_exception(self):
        ...
        action_success = FileAction(path="success.py", action="create", changes=[ChangeBlock(description="create success", content="print(1)")])
        
        # Test loop with both a success action and a failing action
        with patch("application.services.preview_analyzer._analyze_file_action") as mock_analyze:
            row_success = PreviewRow(path="success.py", action="create", description="create success", changes=ChangeSummary(1, 0))
    ...
    def test_calculate_change_summary_actions(self, tmp_path):
        ...
        act_create = FileAction(path="create.py", action="create", changes=[ChangeBlock(description="my create file description", content="content\nlines")])
        ...
        act_rewrite = FileAction(path="rewrite.py", action="rewrite", changes=[ChangeBlock(description="desc", content="new content")])
        ...
        act_modify = FileAction(path="modify.py", action="modify", changes=[ChangeBlock(description="desc", search="find me\nline 2", content="replace content")])
        ...
        act_rename = FileAction(path="rename.py", action="rename", new_path="new_rename.py", changes=[])
    ...
    def test_calculate_rewrite_changes_read_error(self, tmp_path):
        ...
        act = FileAction(path="rewrite.py", action="rewrite", changes=[ChangeBlock(description="desc", content="new content")])
    ...
    def test_generate_description(self):
        ...
        a1 = FileAction(path="a.py", action="create", changes=[ChangeBlock(description="", content="a")])
        assert _generate_description(a1) == "Create file"

        # 13. Rewrite action with/without description (lines 224-225)
        a2 = FileAction(path="a.py", action="rewrite", changes=[ChangeBlock(description="rewrite a", content="a")])
        assert _generate_description(a2) == "rewrite a"
        a2_nodesc = FileAction(path="a.py", action="rewrite", changes=[ChangeBlock(description="", content="a")])
        ...
        # 1 change
        a5_one = FileAction(path="a.py", action="modify", changes=[ChangeBlock(description="desc 1", search="s", content="c")])
        assert _generate_description(a5_one) == "desc 1"
        
        # 2-3 changes
        a5_three = FileAction(path="a.py", action="modify", changes=[
        ...
        # >3 changes
        a5_many = FileAction(path="a.py", action="modify", changes=[
            ChangeBlock(description="desc 1", search="s", content="c"),
            ChangeBlock(description="desc 2", search="s", content="c"),
            ChangeBlock(description="desc 3", search="s", content="c"),
    ...
    def test_generate_preview_diff_lines_edge_cases(self, tmp_path):
        ...
        act_create = FileAction(path="create.py", action="create", changes=[ChangeBlock(description="desc", content="new content")])
        ...
        act_rewrite = FileAction(path="read_err.py", action="rewrite", changes=[ChangeBlock(description="desc", content="new content")])
        with patch.object(Path, "read_text", side_effect=OSError("Read denied")):
        ...
        act_rw = FileAction(path="rw.py", action="rewrite", changes=[ChangeBlock(description="desc", content="hello")])
        ...
        act_rename = FileAction(path="old.py", action="rename", new_path="new.py", changes=[])
        ...
        act_mod_empty = FileAction(path="mod.py", action="modify", changes=[ChangeBlock(description="desc", search=None, content=None)])
        diffs_mod_empty = generate_preview_diff_lines(act_mod_empty, workspace_root=tmp_path)
        assert isinstance(diffs_mod_empty, list)

        # 27. modify action success/failure search/replace (lines 333-348)
        act_mod = FileAction(path="mod.py", action="modify", changes=[ChangeBlock(description="desc", search="find", content="replace")])
        ...
        try:
            ...
            mock_service.apply_search_replace_to_content.return_value = (True, "new content after replace", "")
            diffs_success = generate_preview_diff_lines(act_mod, workspace_root=tmp_path)
